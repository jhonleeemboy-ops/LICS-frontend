...
import BookingModal from '@/components/clients/BookingModal.vue'; // You'll need to create this
...
const useSuggestion = (suggestion) => {
  ...
  // Find relevant topic
  const topic = legalTopics.find(t => 
    suggestion.toLowerCase().includes(t.name.toLowerCase().split(' ')[0])
  ) || legalTopics[0];
...
/* Animation for chat history items */
.border-amber-200 {
  transition: all 0.3s ease;
}

.border-amber-200:hover {
  border-color: #f59e0b;
